...
def abundance_greedy_clustering(amplicon_file: Path, minseqlen: int, mincount: int, chunk_size: int, kmer_size: int) -> List:
    ...
    dereplicated_sequences = list(dereplication_fulllength(amplicon_file, minseqlen, mincount))
    ...
    for seq, count in dereplicated_sequences:
        ...
        for otu_seq, otu_count in otus:
            ...
            alignment = nw.global_align(seq, otu_seq, gap_open=-1, gap_extend=-1, matrix=str(Path(__file__).parent / "MATCH"))
...
def main(): # pragma: no cover
    ...
    otus = abundance_greedy_clustering(amplicon_file, minseqlen, mincount, 0, 0)
